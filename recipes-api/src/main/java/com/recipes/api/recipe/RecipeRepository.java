...
public interface RecipeRepository extends JpaRepository<Recipe, UUID> {
  ...
  @EntityGraph(attributePaths = {"author", "ingredient", "tags"})
  Optional<Recipe> findByPublicIdAndKindAndIngredient(
      String publicId, RecipeKind kind, Ingredient ingredient);

  @EntityGraph(attributePaths = {"author", "ingredient", "tags"})
  @Query(
      """
      select distinct recipeIngredient.recipe
      from RecipeIngredient recipeIngredient
      where recipeIngredient.ingredient = :ingredient
      order by recipeIngredient.recipe.name
      """)
